...
const jsPath = '/tmp/restore-jun23-v2/assets/index-8n3ZwdDs.js';
...
function formatPost(post) {
  ...
  const escapedSubtitle = post.subtitle.replace(/'/g, "\\'");
  const escapedContent = post.content.replace(/\\/g, '\\\\').replace(/`/g, '\\`').replace(/\$\{/g, '\\${');
  const escapedTags = post.tags.map(t => t.replace(/"/g, '\\"')).map(t => `"${t}"`).join(',');
...
const newJsPath = '/tmp/restore-jun23-v2/assets/index-8n3ZwdDs.js';
